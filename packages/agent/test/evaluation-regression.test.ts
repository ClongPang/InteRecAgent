...
import { compareEvaluationReports, parseEvaluationReport } from "../src/evaluation-regression.js";
...
describe("evaluation regression", () => {
  ...
  it("rejects denominator, safety and metric regressions", () => {
    ...
    expect(comparison.failures).toEqual(expect.arrayContaining([
      "metric_denominator_changed:requiredOperationCoverage:12:13",
      "safety_regression:forbiddenOperations:0:1",
    ]));
    expect(comparison.failures.some((failure) => failure.startsWith("metric_regression:requiredOperationCoverage"))).toBe(true);
  });
  ...
  it("marks changed model or prompt versions as incomparable", () => {
    ...
    const comparison = compareEvaluationReports(baseline, current);
    expect(comparison).toMatchObject({ passed: false, comparable: false });
    expect(comparison.failures).toEqual(expect.arrayContaining([
