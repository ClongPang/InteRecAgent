...
import { compareEvaluationReports, parseEvaluationReport, type DriftPolicy } from "./evaluation-drift.js";
...
if (!baselinePath || !currentPath) throw new Error("INTEREC_EVAL_DRIFT_REPORT_PATHS_REQUIRED");

const [baselineJson, currentJson] = await Promise.all([
  readFile(resolve(baselinePath), "utf8"),
  readFile(resolve(currentPath), "utf8"),
]);
const policy: DriftPolicy = process.env["INTEREC_EVAL_DRIFT_POLICY_JSON"]
  ? JSON.parse(process.env["INTEREC_EVAL_DRIFT_POLICY_JSON"])
  : {};
