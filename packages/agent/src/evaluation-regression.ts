...
export interface EvaluationRegressionReport {
  schemaVersion: "interec-eval-regression-v1";
  passed: boolean;
  comparable: boolean;
  baselineImplementationVersion: string;
  currentImplementationVersion: string;
  metrics: Record<EvaluationMetricName, MetricComparison>;
  failures: string[];
}
...
function record(value: unknown, path: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`EVAL_REGRESSION_REPORT_FIELD_INVALID:${path}`);
  return value as Record<string, unknown>;
}

function nonemptyString(value: unknown, path: string): string {
  if (typeof value !== "string" || value.trim() === "") throw new Error(`EVAL_REGRESSION_REPORT_FIELD_INVALID:${path}`);
  return value;
}

function finiteNumber(value: unknown, path: string): number {
  if (typeof value !== "number" || !Number.isFinite(value)) throw new Error(`EVAL_REGRESSION_REPORT_FIELD_INVALID:${path}`);
  return value;
}
...
function parseMetric(value: unknown, path: string): MetricResult {
  ...
  if (parsedValue !== null && (typeof parsedValue !== "number" || !Number.isFinite(parsedValue))) {
    throw new Error(`EVAL_REGRESSION_REPORT_FIELD_INVALID:${path}.value`);
  }
  if (typeof item["passed"] !== "boolean") throw new Error(`EVAL_REGRESSION_REPORT_FIELD_INVALID:${path}.passed`);
  return {
...
export function parseEvaluationReport(value: unknown): EvaluationReport {
  ...
  if (item["schemaVersion"] !== "interec-eval-report-v1") throw new Error("EVAL_REGRESSION_REPORT_SCHEMA_INVALID");
  if (item["mode"] !== "DEVELOPMENT" && item["mode"] !== "HELD_OUT") throw new Error("EVAL_REGRESSION_REPORT_MODE_INVALID");
  if (typeof item["passed"] !== "boolean") throw new Error("EVAL_REGRESSION_REPORT_FIELD_INVALID:report.passed");
  ...
  for (const [family, rawResult] of Object.entries(familyRaw)) {
    ...
    if (typeof result["passed"] !== "boolean") throw new Error(`EVAL_REGRESSION_REPORT_FIELD_INVALID:report.familyStableTasks.${family}.passed`);
  ...
  if (!Array.isArray(item["trials"]) || !Array.isArray(item["failures"])) throw new Error("EVAL_REGRESSION_REPORT_FIELD_INVALID:report.collections");
  return {
...
export function compareEvaluationReports(
  ...
  const metrics = {} as Record<EvaluationMetricName, MetricComparison>;
  for (const name of METRIC_NAMES) {
    ...
    if (!Number.isFinite(tolerance) || tolerance < 0 || tolerance > 1) throw new Error(`EVAL_REGRESSION_POLICY_TOLERANCE_INVALID:${name}`);
    ...
    if (comparable && denominatorChanged) failures.push(`metric_denominator_changed:${name}:${baselineMetric.denominator}:${currentMetric.denominator}`);
