...
import type { AssistantEnvelope, ClaimEvidenceRef, GroundedClaimSet, GroundedClaim, WorkingSet } from "./conversation-types.js";

export interface GroundedClaimValidatorContext {
...
function validateEvidenceRefs(claim: GroundedClaim, allowedEvidenceRefs?: ReadonlySet<string>): void {
...
function validateClaimAgainstCandidate(claim: GroundedClaim, workingSet: WorkingSet): void {
  if (claim.kind === "FX") {
...
function validateComparisonBlocks(ledger: GroundedClaimSet, context: GroundedClaimValidatorContext): void {
  ...
  for (const block of context.envelope.blocks) {
    ...
    if (new Set(scopes).size !== 1) {
      throw new DomainError("COMPARISON_SCOPE_MISMATCH", `Comparison offers do not share a source-grounded category and item role: ${offerRefs.join(",")}`);
    }
  }
}

export function validateGroundedClaimSet(ledger: GroundedClaimSet, context: GroundedClaimValidatorContext): GroundedClaimSet {
  ...
  for (const claim of ledger.claims) {
    ...
    validateEvidenceRefs(claim, context.allowedEvidenceRefs);
    validateClaimAgainstCandidate(claim, context.workingSet);
  }
  validateComparisonBlocks(ledger, context);
