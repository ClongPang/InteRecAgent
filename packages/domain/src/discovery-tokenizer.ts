...
/** Language-independent deterministic tokens suitable for PostgreSQL text[]. */
export function tokenizeDiscoveryText(value: string, limit = 128): string[] {
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > 128) throw new Error("INVALID_DISCOVERY_TOKEN_LIMIT");
...
export interface DiscoveryTokenMatch {
  matchedTokens: string[];
  coverage: number;
}

export function matchDiscoveryTokens(candidateTokens: readonly string[], queryTokens: readonly string[]): DiscoveryTokenMatch {
