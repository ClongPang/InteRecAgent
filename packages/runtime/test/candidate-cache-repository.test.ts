...
import { PostgresCandidateCacheRepository } from "../src/candidate-cache-repository.js";
...
describe("Postgres observed-candidate projection", () => {
  it("upserts a normalized owner-scoped candidate in one transaction", async () => {
    ...
    const repository = new PostgresCandidateCacheRepository(harness.pool);
  ...
  it("hydrates fresh source artifacts for source-grounded local reuse", async () => {
    ...
    await expect(repository.loadArtifacts(owner, ["buywhere:US:laptop-1"], "2026-08-27T12:00:00.000Z")).resolves.toEqual([{
      ...
      artifactRef: `sha256:${createHash("sha256").update(JSON.stringify(payload)).digest("hex")}`,
      rawPayload: payload,
      observedAt: "2026-08-27T00:00:00.000Z",
    }]);
