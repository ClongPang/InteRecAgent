...
const DEFAULT_LIMITS: ProviderCallLimits = {
...
export class ProviderCallControlError extends Error {
  public constructor(public readonly code: string) {
    super(code);
    this.name = "ProviderCallControlError";
  }
}
...
export class PostgresProviderCallController {
  ...
  public async acquire(context: ProviderCallContext): Promise<string> {
    ...
    try {
      ...
      if (Number(count.cluster_active) >= this.limits.clusterConcurrency) throw new ProviderCallControlError("PROVIDER_BULKHEAD_FULL");
      if (Number(count.tenant_active) >= this.limits.tenantConcurrency) throw new ProviderCallControlError("TENANT_PROVIDER_CONCURRENCY_EXCEEDED");
      if (Number(count.minute_requests) >= this.limits.tenantRequestsPerMinute) throw new ProviderCallControlError("TENANT_PROVIDER_RPM_EXCEEDED");
      if (Number(count.day_requests) >= this.limits.tenantRequestsPerDay) throw new ProviderCallControlError("TENANT_PROVIDER_DAILY_QUOTA_EXCEEDED");
