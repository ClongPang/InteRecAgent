...
export function bindRuntimeMetrics(): void {
  ...
  const instruments = {
    ...
    telemetryExportLifecycle: meter.createCounter("rec_agent.telemetry.export_lifecycle", { unit: "{event}" }),
    traceCausalityChecks: meter.createCounter("rec_agent.trace.causality_checks", { unit: "{check}" }),
    traceCausalityViolations: meter.createCounter("rec_agent.trace.causality_violations", { unit: "{violation}" }),
