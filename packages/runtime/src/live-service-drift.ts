export interface LiveServiceDriftConfiguration {
...
export interface LiveServiceDriftSample {
  runIndex: number;
  buyWhere: LiveBuyWhereSample;
  model: LiveModelSample;
}

export interface LiveServiceDriftReport {
  schemaVersion: "interec-live-service-drift-v1";
  generatedAt: string;
  configuration: LiveServiceDriftConfiguration;
  passed: boolean;
  comparableToBaseline: boolean;
  samples: LiveServiceDriftSample[];
  summary: {
...
export interface LiveServiceDriftInput {
  generatedAt: string;
  configuration: LiveServiceDriftConfiguration;
  samples: LiveServiceDriftSample[];
  baseline?: LiveServiceDriftReport;
}

function baselineRecord(value: unknown, path: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`LIVE_DRIFT_BASELINE_INVALID:${path}`);
  return value as Record<string, unknown>;
}

function baselineText(value: unknown, path: string): string {
  if (typeof value !== "string" || value.trim() === "") throw new Error(`LIVE_DRIFT_BASELINE_INVALID:${path}`);
  return value.trim();
}

function baselineStrings(value: unknown, path: string): string[] {
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== "string" || entry.trim() === "")) throw new Error(`LIVE_DRIFT_BASELINE_INVALID:${path}`);
  return value.map((entry) => entry.trim());
}

function baselineInteger(value: unknown, path: string): number {
  if (!Number.isSafeInteger(value) || Number(value) < 0) throw new Error(`LIVE_DRIFT_BASELINE_INVALID:${path}`);
  return Number(value);
}

export function parseLiveServiceDriftBaseline(value: unknown): LiveServiceDriftReport {
  const item = baselineRecord(value, "report");
  if (item["schemaVersion"] !== "interec-live-service-drift-v1") throw new Error("LIVE_DRIFT_BASELINE_SCHEMA_INVALID");
  if (typeof item["passed"] !== "boolean") throw new Error("LIVE_DRIFT_BASELINE_INVALID:report.passed");
  ...
  if (market !== "US" && market !== "SG") throw new Error("LIVE_DRIFT_BASELINE_INVALID:report.configuration.market");
  const runs = baselineInteger(configuration["runs"], "report.configuration.runs");
  if (runs < 2) throw new Error("LIVE_DRIFT_BASELINE_INVALID:report.configuration.runs");
  const summary = baselineRecord(item["summary"], "report.summary");
  if (!Array.isArray(item["samples"]) || !Array.isArray(item["failures"])) throw new Error("LIVE_DRIFT_BASELINE_INVALID:report.collections");
  return {
    schemaVersion: "interec-live-service-drift-v1",
    ...
    samples: item["samples"] as LiveServiceDriftSample[],
    summary: {
...
export function evaluateLiveServiceDrift(input: LiveServiceDriftInput): LiveServiceDriftReport {
  ...
  for (const sample of input.samples) {
    ...
    if (!sample.model.ok) failures.push(`model_failed:${sample.runIndex}:${sample.model.errorCode ?? "UNKNOWN"}`);
      ...
      if (sample.model.provider !== input.configuration.modelProvider) failures.push(`model_provider_drift:${sample.runIndex}:${sample.model.provider ?? "null"}`);
      if (sample.model.requestedModel !== input.configuration.modelId) failures.push(`model_id_drift:${sample.runIndex}:${sample.model.requestedModel ?? "null"}`);
  ...
  if (input.baseline) {
    ...
    if (!compatible && baseline.passed) {
      ...
      if (baseline.summary.buyWhereContractFingerprints.length !== 1 || baseline.summary.buyWhereContractFingerprints[0] !== buyWhereContractFingerprints[0]) {
        failures.push(`baseline_buywhere_contract_drift:${baseline.summary.buyWhereContractFingerprints[0] ?? "missing"}:${buyWhereContractFingerprints[0] ?? "missing"}`);
      }
      if (baseline.summary.modelContractFingerprints.length !== 1 || baseline.summary.modelContractFingerprints[0] !== modelContractFingerprints[0]) {
        failures.push(`baseline_model_contract_drift:${baseline.summary.modelContractFingerprints[0] ?? "missing"}:${modelContractFingerprints[0] ?? "missing"}`);
      }
    }
  }
  return {
