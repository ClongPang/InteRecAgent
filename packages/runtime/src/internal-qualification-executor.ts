...
import { qualificationModelFailureCode, type InternalQualificationCase } from "@interec/agent";
...
export interface InternalQualificationTrialArtifact extends Record<string, unknown> {
  trialId: string;
  taskId: string;
  runIndex: number;
...
export interface InternalQualificationTrialExecutorOptions {
  promptLink?: LangfusePromptLink;
}

export interface QualificationExperimentTraceContext {
  datasetRunName: string;
  datasetItemId: string;
  experimentWrapperTraceId: string;
}

export function qualificationImplementationFingerprint(root = process.cwd()): string {
  ...
  try {
    await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeout = setTimeout(() => reject(new Error(`QUALIFICATION_BARRIER_TIMEOUT:${label}`)), 90_000);
      }),
    ]);
  } finally {
...
async function reachedBeforeRunEnds(value: Promise<void>, run: Promise<boolean>, label: string): Promise<void> {
  await Promise.race([
    ...
    run.then(() => { throw new Error(`QUALIFICATION_INTERRUPT_POINT_NOT_REACHED:${label}`); }),
  ]);
}

function fixtureQueryFor(seed: InternalQualificationCase["fixtureSeed"], actualQuery: string): string {
  ...
  throw new Error(`QUALIFICATION_FIXTURE_SEED_UNMAPPED:${seed satisfies never}`);
}

function qualificationProductPort(
  base: ReplayProductSearchPort,
  fixtureSeed: InternalQualificationCase["fixtureSeed"],
  environmentAction: string,
...
async function collectTurnEvidence(repository: PostgresConversationRepository, turnId: string): Promise<Record<string, unknown>> {
  ...
  const research = await repository.pool.query<Record<string, unknown>>(
  ...
  const qualifications = await repository.pool.query<Record<string, unknown>>(
  ...
  return { ...(result.rows[0] ?? { status: "MISSING" }), toolExecutions: tools.rows, research: research.rows, qualificationCounts: qualifications.rows };
}
...
function assertModelProviderAvailable(turnEvidence: Array<Record<string, unknown>>): void {
  for (const evidence of turnEvidence) {
    ...
    const code = qualificationModelFailureCode((draft as Record<string, unknown>)["fallbackReasonCode"]);
    if (code) throw new Error(`QUALIFICATION_${code}`);
  }
}

export class InternalQualificationTrialExecutor {
  public constructor(
    ...
    private readonly options: InternalQualificationTrialExecutorOptions = {},
  ) {}
  ...
  private createWorker(
    ...
    return new ConversationWorker(
      repository,
      new PostgresConversationResearchRepository(this.repository.pool),
      new PostgresProviderGovernor(this.repository.pool),
      productPort,
      fxPort,
      this.pi,
      {
        workerId: `qualification-${suffix}-${randomUUID()}`,
        leaseSeconds: 60,
        heartbeatSeconds: 10,
  ...
  private async resetGovernorState(): Promise<void> {
    await this.repository.pool.query(
      `UPDATE interec_agent.provider_permits
          SET status = 'EXPIRED', completed_at = clock_timestamp(), error_code = 'QUALIFICATION_TRIAL_ENDED'
        WHERE tenant_id LIKE 'qualification-%' AND status = 'ACTIVE'`,
    );
    await this.repository.pool.query(
  ...
  public async execute(
    testCase: InternalQualificationCase,
    runIndex: number,
    experimentTrace?: QualificationExperimentTraceContext,
  ): Promise<InternalQualificationTrialArtifact> {
    if (!Number.isSafeInteger(runIndex) || runIndex < 1) throw new Error("QUALIFICATION_RUN_INDEX_INVALID");
    await this.resetGovernorState();
    ...
    const productPort = qualificationProductPort(productReplay, testCase.fixtureSeed, testCase.environmentAction, queryAliases, providerBarrier);
    const owner = { tenantId: `qualification-${randomUUID()}`, ownerId: "internal-evaluator" };
    const conversation = await this.repository.createConversation(owner);
    const acceptQualificationTurn = async (input: AcceptConversationTurnInput) => {
    ...
    try {
      if (testCase.environmentAction.startsWith("INTERRUPT_")) {
        const first = await acceptQualificationTurn({
        ...
        const second = await acceptQualificationTurn({
          conversationId: conversation.id,
          owner,
        ...
        for (const [turnOffset, content] of testCase.userTurns.entries()) {
          ...
          if (!projectionBefore) throw new Error("QUALIFICATION_PROJECTION_MISSING");
          const focusOfferRef = turnOffset > 0 && testCase.focusDisplayRank !== undefined
            ...
            : undefined;
          const accepted = await acceptQualificationTurn({
            conversationId: conversation.id,
            owner,
          ...
          if (!completed || (completed.status !== "COMPLETED" && completed.status !== "FAILED")) throw new Error(`QUALIFICATION_TURN_NOT_TERMINAL:${accepted.id}`);
          if (completed.status === "FAILED") throw new Error(`QUALIFICATION_TURN_FAILED:${completed.errorCode ?? "UNKNOWN"}`);
          await collectOutstandingTurnEvidence();
          assertModelProviderAvailable(turnEvidence);
        }
      }
