...
export function resolveBuyWhereTimeoutMs(environment: NodeJS.ProcessEnv = process.env): number {
  const raw = retailPriceEnvironmentValue(environment, "PROVIDER_BUYWHERE_TIMEOUT_MS")?.trim();
  if (!raw) return DEFAULT_BUYWHERE_TIMEOUT_MS;
  if (!/^\d+$/.test(raw)) throw new Error("RETAIL_PRICE_PROVIDER_BUYWHERE_TIMEOUT_MS_INVALID");
  const value = Number(raw);
  if (value < MIN_BUYWHERE_TIMEOUT_MS || value > MAX_BUYWHERE_TIMEOUT_MS) {
    throw new Error("RETAIL_PRICE_PROVIDER_BUYWHERE_TIMEOUT_MS_OUT_OF_RANGE");
  }
  return value;
}
...
export function resolveBuyWhereRuntimeConfig(environment: NodeJS.ProcessEnv = process.env): BuyWhereRuntimeConfig {
  const apiKey = retailPriceEnvironmentValue(environment, "PROVIDER_BUYWHERE_API_KEY")?.trim() ?? "";
  if (!apiKey) throw new Error("RETAIL_PRICE_PROVIDER_BUYWHERE_API_KEY_REQUIRED");
