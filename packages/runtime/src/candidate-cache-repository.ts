...
import type { CategoryValidationMode } from "@interec/domain";
...
export interface CandidateCacheSearch {
  tokens: string[];
  markets?: string[];
  limit?: number;
  now?: string;
...
export interface RankedCachedCandidate extends CachedCandidate {
  matchedTokenCount: number;
  tokenCoverage: number;
}

export interface CandidateCacheRepository {
  upsert(owner: OwnerClaims, candidates: readonly CachedCandidate[]): Promise<number>;
  search(owner: OwnerClaims, input: CandidateCacheSearch): Promise<RankedCachedCandidate[]>;
...
type CandidateRow = {
  ...
  support_level: CategoryValidationMode;
...
function boundedTokens(tokens: readonly string[]): string[] {
  ...
  if (values.length < 1 || values.length > 128) throw new Error("INVALID_CACHED_CANDIDATE_TOKENS");
  return values;
}

function timestamp(value: Date | string): string {
  const date = value instanceof Date ? value : new Date(value);
  if (Number.isNaN(date.getTime())) throw new Error("INVALID_CACHED_CANDIDATE_TIMESTAMP");
  return date.toISOString();
}

function candidateFromRow(row: CandidateRow, queryTokenCount: number): RankedCachedCandidate {
  ...
  return {
    ...
    validationMode: row.support_level,
...
export class PostgresCandidateCacheRepository implements CandidateCacheRepository {
  public constructor(private readonly pool: pg.Pool) {}

  public async upsert(owner: OwnerClaims, candidates: readonly CachedCandidate[]): Promise<number> {
    ...
    return withOwnerTransaction(this.pool, owner, async (client) => {
      ...
      for (const candidate of candidates) {
        ...
        if (Date.parse(expiresAt) <= Date.parse(observedAt)) throw new Error("INVALID_CACHED_CANDIDATE_EXPIRY");
  ...
  public async search(owner: OwnerClaims, input: CandidateCacheSearch): Promise<RankedCachedCandidate[]> {
    ...
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100) throw new Error("INVALID_CACHED_CANDIDATE_LIMIT");
