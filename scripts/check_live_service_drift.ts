...
import {
  ...
  evaluateLiveServiceDrift,
  parseLiveServiceDriftBaseline,
  resolveBuyWhereRuntimeConfig,
  type LiveBuyWhereSample,
  type LiveModelSample,
} from "../packages/runtime/src/index.js";

if (process.env["INTEREC_LIVE_DRIFT_CONFIRM"] !== "authorized-external-drift") {
  throw new Error("INTEREC_LIVE_DRIFT_CONFIRM_MUST_BE_authorized-external-drift");
}
...
function buyWhereContractFingerprint(_rawPayload: unknown, _products: unknown[]): string {
  ...
  // data variation, not API schema drift.
  return fingerprint({
...
const configuration = {
  ...
  query: process.env["INTEREC_LIVE_DRIFT_QUERY"]?.trim() || "Sony WH-1000XM5 headphones",
  market: (process.env["INTEREC_LIVE_DRIFT_MARKET"]?.trim().toUpperCase() || "US") as "US" | "SG",
  runs: boundedInteger("INTEREC_LIVE_DRIFT_RUNS", 3, 2, 10),
};
if (configuration.market !== "US" && configuration.market !== "SG") throw new Error("INTEREC_LIVE_DRIFT_MARKET_INVALID");
...
const baselinePath = process.env["INTEREC_LIVE_DRIFT_BASELINE_PATH"]?.trim();
const baseline = baselinePath
  ? parseLiveServiceDriftBaseline(JSON.parse(await readFile(resolve(baselinePath), "utf8")))
  : undefined;
const report = evaluateLiveServiceDrift({
...
const reportPath = process.env["INTEREC_LIVE_DRIFT_REPORT_PATH"]?.trim();
if (reportPath) {
