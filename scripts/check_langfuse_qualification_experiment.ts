...
const artifactPath = resolve(process.env["INTEREC_LANGFUSE_EXPERIMENT_ARTIFACT"] ?? ".artifacts/evaluation/internal-qualification-langfuse-real-117.json");
...
try {
  ...
  for (const run of runs) {
    ...
    summaries.push({ runName, datasetRunId: expectedRunId, items: traceIds.length, scores: scores.length, values });
  }
