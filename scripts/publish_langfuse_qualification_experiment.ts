...
import {
  ...
  startTelemetry,
  type InternalQualificationTrialArtifact,
...
function boundedRuns(): number {
  const runs = Number(process.env["INTEREC_INTERNAL_QUALIFICATION_RUNS"]?.trim() || 3);
  if (!Number.isSafeInteger(runs) || runs < 1 || runs > 3) throw new Error("INTEREC_INTERNAL_QUALIFICATION_RUNS_INVALID");
  return runs;
}

function boundedCaseLimit(maximum: number): number {
  const limit = Number(process.env["INTEREC_INTERNAL_QUALIFICATION_CASE_LIMIT"]?.trim() || maximum);
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > maximum) throw new Error("INTEREC_INTERNAL_QUALIFICATION_CASE_LIMIT_INVALID");
  return limit;
}
...
const outputPath = resolve(process.env["INTEREC_INTERNAL_QUALIFICATION_OUTPUT"] ?? ".artifacts/evaluation/internal-qualification-runs-v2.json");
...
if (selectedCases.length === 0) throw new Error("QUALIFICATION_TASK_FILTER_EMPTY");
...
const telemetry = await startTelemetry("interec-qualification-real-experiment");
...
const executor = new InternalQualificationTrialExecutor(repository, fixture, pi, { promptLink: nativePromptLink });
const caseByTaskId = new Map(selectedCases.map((entry) => [entry.taskId, entry]));
const output = {
  schemaVersion: "interec-internal-qualification-runs-v2",
  ...
  trials: [] as InternalQualificationTrialArtifact[],
};
...
const experimentScoreNames = [
  "qualification_business_pass",
  "qualification_protocol_clean",
  "qualification_expected_outcome",
  "qualification_state_consistent",
  "qualification_trace_complete",
] as const;
...
async function publishAndVerifyScores(
  ...
  for (const itemResult of result.itemResults) {
    ...
    const repaired = await retryLangfuseIdempotentRequest(() => langfuse.api.datasetRunItems.create({
      ...
      datasetItemId: itemResult.item.id,
  ...
  const batch = result.itemResults.flatMap((itemResult) => {
    ...
    const values = {
      qualification_business_pass: evaluated["passed"] === true,
      ...
      qualification_trace_complete: checks["traceComplete"] === true,
    };
    return experimentScoreNames.map((name) => {
      const scoreId = deterministicUuidV5(`experiment-score\0${datasetRunId}\0${itemResult.traceId}\0${name}`);
      return {
        ...
        body: {
          ...
          comment: `qualification-v1: ${name}=${values[name] ? 1 : 0}`,
          metadata: {
            datasetRunId,
            runIndex,
            implementationSha256,
...
try {
  ...
  for (let runIndex = 1; runIndex <= runs; runIndex += 1) {
    const runName = `internal-qualification-${String(pi.model.id)}-${implementationSha256.slice(7, 15)}-${executionTag}-rep-${runIndex}`;
    ...
    const result = await langfuse.experiment.run({
      name: "InteRecAgent real internal qualification",
      runName,
      ...
      task: async (item) => {
        ...
        persist();
        const evaluation = evaluateQualificationExperimentTrial(trial, expectedOutput);
  ...
  process.stdout.write(`${JSON.stringify({ mode: "EXECUTED", failed, ...plan, datasetRuns: output.datasetRuns }, null, 2)}\n`);
