...
2026-08-20 收口：配件（HDMI 线、日记本）即使标题带 Monitor 也丢掉。品类线索全空时不再把脏召回整表灌回；英文型号名靠检索词片段（27 / 4K）弱匹配。对照上限改为 6 件（`MAX_RANKED_CANDIDATES` / `ResearchLimits.top_k`）。研究子图改为后端控环：规则过滤后模型 keep，再搜并入池子，池子 ≥ 25 或满 3 次检索后由模型选 TopK。
