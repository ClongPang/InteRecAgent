...
**文档类型**：问题复盘 + 方案取舍  
**范围**：研究子图里注册给 LLM 的工具签名。记录为什么只有检索和收束带参数，换算 / 过滤 / 排序故意无参。
...
只在 `ResearchTools.specs` 注册，经 `run_agent` 调用 `backend.chat(..., tools=tools.specs)`，`tool_choice=auto`。`supports_tools()` 为假时，确定性驱动按固定顺序调用**同一批**执行器。
...
系统提示写在 `backend/agent/loop.py`：先检索，再换算，再过滤，最后排序；空集或过少则放宽后重搜；事实由工具返回，不要编造；禁止完全相同的重复调用。
...
这与提示词一致：「你只负责决定调用哪个工具、传什么参数；价格、库存、汇率、链接等事实由工具返回。」
...
| 让模型在 finalize 前再筛一遍 | 少写过滤器 | 贵、慢、会编；品类门闩必须在 `run_filter` |
| 无 Key 时另写一套线性函数 | 实现快 | 两套流水线会漂移；现行是同一执行器、两种驱动 |

确定性安全网：`run_agent` 跑满步数或半途终止且 `ctx.ranked` 仍空时，调用 `run_deterministic`（检索 → 换算 → 过滤 → 排序；必要时放宽原币上限或库存软条件）。运行已过期（`version_probe`）则不补全。
...
| 当轮候选总线 | `backend/agent/tools/context.py` |
| LLM 循环 / 确定性驱动 / 系统提示 | `backend/agent/loop.py` |
| 研究节点挂载 | `backend/agent/nodes/research.py` |
| 硬过滤 / 汇率 / 排序流水线 | `backend/application/services/rec/pipeline.py` |
| OpenAI 兼容 tool calling | `backend/infrastructure/llm/openai_compat.py` |
