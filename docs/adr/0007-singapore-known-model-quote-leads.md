...
- 状态：Accepted（原 Accepted for implementation 已完成）
- 日期：2026-09-01
- 替代：ADR-0004 中跨市场商品推荐业务语义
- 保留：ADR-0004 的 durable Conversation Turn Runtime，以及 ADR-0003 的来源充分性原则

## 决策

RetailPriceAgent 的唯一首发产品边界是：面向新加坡市场，针对用户已经知道的商品型号，查询 BuyWhere 报价记录，经过确定性型号与商品角色校验后，提供报价线索和商家页入口。
...
`search_products_v2` 不得作为用户报价链路的自动 fallback。REST `sort=price_asc` 同样不得作为静默 fallback。
...
以下能力不因业务域调整而重写：PostgreSQL 权威状态、Turn lease/fencing、幂等 Provider ledger、attempt-scoped draft、原子发布、RLS、outbox、以及 `commit_quote_plan` 单工具协议（模型提案，宿主执行与渲染）。

## 不兼容迁移

旧跨市场推荐会话不得自动解释为新加坡报价会话。新会话写入 `quote-leads-sg-v1`；旧会话只读或引导用户开始新会话。活动路径只有一个实现，不保留运行时 fallback 或双写。

## 验收依据

机器可读产品合同为 `spec/quote-lead-product-contract.json`。阶段状态为 `spec/quote-lead-refactor-state.json`（phase 5 APPROVED）。漂移门禁见 `docs/quote-lead-refactor-execution-plan.md`。
