...
商品目录是开放世界，任何基于历史 bad case 扩展的负向规则都不可能完备。问题不在某个关键词，而在候选准入和回复信息发布模型。

## 决策

采用基于来源充分性的白名单准入和类型状态流水线。文中保留的 `proof-carrying-*` 仅是历史内部策略版本标识，不代表形式化证明系统：

```text
RawArtifact
  -> DiscoveredListing
  -> ProductIdentity
  -> CandidateEligibilityResult（内部类型仍为 ListingEligibilityResult）
  -> ComparableOffer
  -> RankedOfferSet
  -> ValidatedDecision
```
...
每个规范化来源字段使用内部类型 `Fact<T>`，包含值、状态和来源引用 `EvidenceRef[]`。市场语义明确拆分为：
...
商品身份使用类别、规范型号、`itemRole` 和 condition 组成 comparison key。系统比较同一 comparison key；不同本体/配件、型号或 condition 不进入同一个 RankedOfferSet。

BuyWhere 定位为商品搜索来源。它提供原始价格、标题和候选链接，但不能据此确认配送资格或最终购买条件。后续若接入独立核验来源，也必须通过相同的来源记录契约接入。
...
- 事实提取和状态晋级；
- RankedOfferSet 构造与排序；
- 可选 Decision、AssistantMessage 与本轮状态在 PostgreSQL 事务内原子提交。
...
- `source_listings`
- `offer_eligibilityResults`
- `comparison_sets`
- `comparison_set_items`
- `decisions.comparison_set_id`

原始 artifact 保持不可变；失败晋级的 listing 和 reason codes 继续进入审计账本，但不会进入 RankedOfferSet。
...
4. item role、规范型号或 condition 不同的结果不得混入同一 RankedOfferSet。
5. 输入排列变化不能改变确定性 Decision。
6. 所有输出中的商品信息必须回溯到来源引用 `EvidenceRef`。
