...
function ChangeSummary({ change, undo }: { change: ChangeRecord | null; undo: () => void }) { if (!change) return null; return <section className="change-summary" aria-live="polite"><div><strong>本次变更</strong><p>{change.summary} · 备选 {change.resultBefore} → {change.resultAfter}</p></div><button onClick={undo}>撤销本次修改</button></section> }
...
function Conversation({ mission, apply, compare, canCompare, comparing = false, messages, append, latestChange, undo }: { mission: Mission; apply: (delta: Partial<Mission>, summary: string) => void; compare: () => void; canCompare: boolean; comparing?: boolean; messages: Message[]; append: (items: Message[]) => void; latestChange: ChangeRecord | null; undo: () => void }) {
  ...
  const action = (name: string) => {
    if (name === '比较已选商品') { if (canCompare) compare(); else append([{ id: String(Date.now()), role: 'agent', text: '先加入两件备选，再开始对比。' }]); return }
  ...
  const submit = (event?: FormEvent) => {
    ...
    else append([{ id: `a-${Date.now()}`, role: 'agent', text: `我还不能用“${text}”改变排序，因为当前备选没有对应的可比字段。你可以调整预算、库存、商品类别，或选择可用的排序条件。` }])
    setDraft('')
  }
  const audioActions = supportsAudioPriorities(mission) ? ['优先降噪', '优先续航', '先看综合推荐'] : ['按商品价排序', '先看综合推荐']
  return <aside className={`conversation-panel ${messages.length <= 1 ? 'is-brief' : ''}`}><div className="conversation-header"><div><span>选购助手</span></div></div><Brief mission={mission} /><ChangeSummary change={latestChange} undo={undo} /><div className="conversation-thread" aria-label="选购对话">{messages.map((message) => <div className={`conversation-message ${message.role}`} key={message.id}><div className="message-meta">{message.role === 'user' ? '你' : '选购助手'}</div><p>{message.text}</p>{message.actions && <div className="message-actions">{message.actions.map((name) => <button key={name} onClick={() => action(name)}>{name}</button>)}</div>}</div>)}</div><div className="conversation-suggestions">{audioActions.map((name) => <button key={name} onClick={() => action(name)}>{name}</button>)}<button onClick={() => apply({ onlyInStock: !mission.onlyInStock }, mission.onlyInStock ? '库存：显示全部' : '库存：仅看有货')}>{mission.onlyInStock ? '显示全部库存' : '只看有货'}</button>{!comparing && <button onClick={() => action('比较已选商品')}>对比所选</button>}</div><form className="conversation-composer" onSubmit={submit}><textarea rows={2} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="例如：预算改为 ¥2000，或只看有货" aria-label="选购对话输入" /><div className="composer-row"><button className="send-button" disabled={!draft.trim()} aria-label="发送"><Icon name="arrow" size={16} /></button></div></form></aside>
}
...
function Decision({ candidates, mission, currency = 'RMB' }: { candidates: Product[]; mission: Mission; currency?: Currency }) { const product = candidates[0]; if (!product) return null; const explanation = mission.preference === 'battery' && batteryHours(product) ? `续航 ${batteryHours(product)} 小时，是当前可推荐备选中最长的。` : mission.preference === 'noise' && product.brand === 'Bose' ? '它的降噪取向与当前偏好最一致。' : mission.preference === 'lowest' ? '它是当前可推荐备选中商品参考价最低的一件。' : product.why; return <section className="decision-card"><div className="decision-icon"><Icon name="spark" size={20} /></div><div className="decision-copy"><span>当前推荐</span><h2>{product.brand} {product.model}</h2><p>{explanation} {product.tradeoff}</p><div className="decision-tags">{[mission.budget ? '商品价在预算内' : '未设置预算', mission.onlyInStock ? '当前有货' : '库存已列出', preferenceText(mission.preference)].map((tag) => <span key={tag}>{tag}</span>)}</div></div><div className="decision-price"><span>商品价估算</span><strong>{CURRENCY_SYMBOL[currency]}{priceIn(product.rmbPrice, currency).toLocaleString()}</strong><small>{product.nativePrice} · {product.updated}</small></div></section> }
        ...
        <section className="results-region">
          ...
          {unsupported ? (
            <section className="empty-result">
              ...
              <Button onClick={() => apply({ intent: '通勤降噪耳机' }, '商品：通勤降噪耳机')}>查看耳机备选</Button>
            </section>
          ) : (
            <>
              <DecisionOverview candidates={candidates} eligibleCount={recommended.length} platformCount={platformCount} mission={mission} />
              {recommended.length ? (
                <Decision candidates={recommended} mission={mission} currency={currency} />
              ) : (
                <section className="empty-result">
                  <Icon name="search" size={24} />
                  <h2>预算内暂无符合条件的备选</h2>
              ...
              <div className="filter-bar">
                ...
                <div className="filter-actions">
                  ...
                  <select value={mission.preference} onChange={(event) => apply({ preference: event.target.value as Preference }, `排序：${preferenceText(event.target.value as Preference)}`)} aria-label="备选排序">
...
function Compare({ mission, selectedIds, back, detail, apply, onRemove, messages, append, latestChange, undo, currency }: { mission: Mission; selectedIds: string[]; back: () => void; detail: (product: Product) => void; apply: (delta: Partial<Mission>, summary: string) => void; onRemove: (id: string) => void; messages: Message[]; append: (items: Message[]) => void; latestChange: ChangeRecord | null; undo: () => void; currency: Currency }) {
  ...
  return <main className="workspace-view compare-view">
    ...
    <div className="workspace-layout">
      <Conversation mission={mission} apply={apply} compare={() => {}} canCompare={true} comparing messages={messages} append={append} latestChange={latestChange} undo={undo} />
      <section className="results-region">
        ...
        <section className="decision-card compare-decision"><div className="decision-icon"><Icon name="spark" size={20} /></div><div className="decision-copy"><span>当前推荐</span><h2>{lead.brand} {lead.model}</h2><p>{explanation}</p><small className="decision-boundary">推荐基于商品价与可见规格；运费、税费与配送资格以商户结算页为准。</small></div><div className="decision-price"><span>商品价估算</span><strong>{CURRENCY_SYMBOL[currency]}{priceIn(lead.rmbPrice, currency).toLocaleString()}</strong><small>{lead.nativePrice} · {lead.updated}</small></div></section>
...
function TaskList({ tasks, activeId, open, create }: { tasks: ShoppingTask[]; activeId: number | null; open: (id: number) => void; create: () => void }) { return <main className="task-list-view"><div className="task-list-header"><div><h1>我的选购</h1><p>每笔选购保留自己的对话、条件、备选与比较进度。</p></div><Button variant="primary" onClick={create} icon="plus">新建选购</Button></div>{tasks.length ? <section className="task-list" aria-label="已保存的选购">{tasks.map((task) => <button className={`task-list-item ${task.id === activeId ? 'is-current' : ''}`} key={task.id} onClick={() => open(task.id)}><div><strong>{task.mission.intent}</strong><p>{budgetText(task.mission)} · {preferenceText(task.mission.preference)}{task.mission.onlyInStock ? ' · 仅看有货' : ''}</p></div><div className="task-list-meta"><span>{task.id === activeId ? '当前处理' : phaseText(task.phase, task.selected)}</span><small>{task.updatedAt} 更新</small></div><Icon name="arrow" size={16} /></button>)}</section> : <section className="task-list-empty"><Icon name="spark" size={24} /><h2>还没有选购</h2><p>新建一笔选购后，可以随时在这里继续对话和比较。</p><Button variant="primary" onClick={create}>新建选购</Button></section>}</main> }
function QuickSwitcher({ tasks, activeId, open, close, allTasks, create }: { tasks: ShoppingTask[]; activeId: number | null; open: (id: number) => void; close: () => void; allTasks: () => void; create: () => void }) { const [query, setQuery] = useState(''); const items = [...tasks].sort((a, b) => b.updatedAtMs - a.updatedAtMs).filter((task) => `${task.mission.intent} ${budgetText(task.mission)} ${preferenceText(task.mission.preference)}${task.mission.onlyInStock ? ' 仅看有货' : ''}`.includes(query.trim())); return <div className="switcher-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}><section className="task-switcher" role="dialog" aria-modal="true" aria-label="切换选购"><div className="switcher-heading"><div><strong>切换选购</strong><small>将恢复各选购上次的工作现场</small></div><button onClick={close} aria-label="关闭选购切换"><Icon name="close" size={16} /></button></div><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索选购名称、预算或偏好" aria-label="搜索选购" />{items.length ? <div className="switcher-list">{items.slice(0, 6).map((task) => <button className={task.id === activeId ? 'is-current' : ''} key={task.id} onClick={() => open(task.id)}><span><b>{task.mission.intent}</b><small>{budgetText(task.mission)} · {preferenceText(task.mission.preference)}{task.mission.onlyInStock ? ' · 仅看有货' : ''} · {task.updatedAt}</small></span>{task.id === activeId && <em>当前</em>}</button>)}</div> : <p className="switcher-empty">未找到匹配的选购</p>}<div className="switcher-footer"><button onClick={allTasks}>查看全部选购</button><button onClick={create}>新建选购</button></div></section></div> }
...
export default function App() {
  ...
  const toggle = (id: string) => updateActive((task) => { const selected = task.selected.includes(id) ? task.selected.filter((item) => item !== id) : task.selected.length >= 4 ? task.selected : [...task.selected, id]; return { ...task, selected, phase: selected.length ? 'shortlisting' : 'collecting', lastSurface: 'discover', updatedAt: nowText(), updatedAtMs: Date.now() } })
